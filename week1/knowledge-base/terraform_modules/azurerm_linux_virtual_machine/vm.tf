resource "azurerm_linux_virtual_machine" "vm" {
  ...
  admin_password = var.ssh_public_key != null ? null : azurerm_key_vault_secret.vm_secret[0].value

  custom_data = var.custom_data
  user_data   = var.user_data

  disable_password_authentication = var.ssh_public_key == null ? false : true
