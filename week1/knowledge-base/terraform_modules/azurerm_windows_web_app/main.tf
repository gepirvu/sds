...
resource "azurerm_windows_web_app" "win" {
  ...
  dynamic "site_config" {
    ...
    content {
      application_stack {
        current_stack  = each.value.application_stack
  ...
  dynamic "site_config" {
    for_each = each.value.application_stack == "node" ? [1] : []
    content {
      application_stack {
        current_stack = each.value.application_stack
  ...
  dynamic "site_config" {
    for_each = each.value.application_stack == "" ? [1] : []
    content {
...
resource "azurerm_role_assignment" "pull_image" {
  for_each = {
    for key, app in azurerm_windows_web_app.win :
    key => app
...
resource "azurerm_windows_web_app_slot" "deployment_slot" {
  ...
  app_service_id                = azurerm_windows_web_app.win[each.value.appservice_short_description].id
  https_only                    = true
  client_affinity_enabled       = each.value.client_affinity_enabled
  public_network_access_enabled = false
  app_settings                  = lookup(var.app_settings, each.value.app_settings_name, null)
...
resource "azurerm_app_service_virtual_network_swift_connection" "azure_vnet_connection" {
  for_each       = azurerm_windows_web_app.win
...
resource "azurerm_private_endpoint" "private_endpoint_slots" {
  for_each            = { for n in var.deployment_slots : n.deployment_slot_name => n }
  depends_on          = [azurerm_windows_web_app_slot.deployment_slot]
  ...
  private_service_connection {
    ...
    private_connection_resource_id = azurerm_windows_web_app.win[each.value.appservice_short_description].id
    is_manual_connection           = false
