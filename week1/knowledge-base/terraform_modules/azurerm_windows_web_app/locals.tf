locals {
  ...
  app_services_with_acr = {
    ...
    if app.use_acr && app.identity_type == "SystemAssigned"
  }
...
locals {
  ...
  verification_id = {
    ...
    config.webapp_description => azurerm_windows_web_app.win[config.webapp_description].custom_domain_verification_id
  }
