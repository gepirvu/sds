...
resource "azurerm_private_endpoint" "private_endpoint" {
  for_each            = { for config in var.function_app : config.usecase => config }
  name                = "${azurerm_windows_function_app.function_app[each.value.usecase].name}-pe"
  location            = var.location
  resource_group_name = var.resource_group_name
  subnet_id           = data.azurerm_subnet.pe_subnet.id

  private_service_connection {
    name                           = "${azurerm_windows_function_app.function_app[each.value.usecase].name}-pe-sc"
    private_connection_resource_id = azurerm_windows_function_app.function_app[each.value.usecase].id
